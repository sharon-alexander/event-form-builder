...
export function locationConfigFromRow(row: LocationRow): LocationConfig {
  return {
    ...
    allowMultipleVenueSpaces: row.allow_multiple_venue_spaces ?? false,
    showMultiDayRental: row.show_multi_day_rental ?? false,
    showAdditionalLoadInOut: row.show_additional_load_in_out ?? false,
    showFullDayRental: row.show_full_day_rental ?? false,
    ...
    steps: row.form_steps && row.form_steps.length > 0 ? row.form_steps : [],
    ...
    fieldSettings: resolveFieldSettings(row.field_settings, row.required_fields),
    tripleseat: {
...
export async function fetchLocationBySlug(
  ...
  let query = client.from("locations").select("*").eq("slug", slug);
  if (!preview) {
